...
const Icon: FC<IconProps> = ({ id, open }) => {
    return (
        <div className="w-[30px] h-[30px] bg-[#A0AEFF] rounded-full flex items-center justify-center">
            <svg
        ...
        <>
            {items.map((item) => (
                <Accordion key={item.id} open={open === item.id} icon={<Icon id={item.id} open={open} />}>
                    <AccordionHeader onClick={() => handleOpen(item.id)} className="text-[21px] font-normal leading-[27px] text-[#3A3A3A]">{item.title}</AccordionHeader>
